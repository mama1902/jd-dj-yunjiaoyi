...
class Frontend extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $shopInfo = Db::name("cms_shopinfo")
            ->where('id', $this->auth->vender_id)
            ->find();
        $this->merchant_id = $shopInfo['merchant_id'] ?? 0;

        $shopinfo = new  \app\admin\model\Shopinfo;
        $verder_list = $shopinfo->where('merchant_id', $this->merchant_id )->column('id,station_name');


        $this->view->assign('shoplist', build_select('vender_id', $verder_list, $this->auth->vender_id, ['class' => ' se2', 'id' => 'se2']));
