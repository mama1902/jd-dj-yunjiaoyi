...
class Command extends Backend
{
    ...
    protected $noNeedRight = ['get_controller_list', 'get_field_list','execute2','orderre'];
    ...
    public function execute2($ids)
    {
        ...
        if ( !empty($order['status_y']) ) {
           // $this->error(__('订单状态不可取消'));
        }
        $output = Console::call('AdaRepair', ['cancelorder', '--money', $order['out_orderid'] ]);


        $this->success("", null, ['result' => $output]);
    }

    /**
     * 执行
     */
    public function orderre($ids)
    {

        $orderobj = new \app\admin\model\Ordertemp;
        $order = $orderobj::where('id', $ids)
            ->order('id', 'desc')
            ->find();
        if (empty($order)) {
            $this->error(__('No Results were found'));
        ...
        $orderobj->where(['id' => $order['id']])->update(['is_create' => 0, 'msg' => '尝试重推,']);

        $this->success("已加入重推队列，请留意", null, []);
    }
